    ...
    def normalize(self, cur=None):
        ...
        if not ('mail' in address_in and address_in['mail']):
            ...
            if 'stnum' in address_in:
                address_in['mail'] = address_in[
                    'mail'] + ' ' + address_in['stnum']
            ...
            if 'stpredirection' in address_in:
                address_in['mail'] = address_in['mail'] + \
                    ' ' + address_in['stpredirection']
            if 'stname' in address_in:
                address_in['mail'] = address_in[
                    'mail'] + ' ' + address_in['stname']
            if 'sttype' in address_in:
                address_in['mail'] = address_in[
                    'mail'] + ' ' + address_in['sttype']
            if 'stpostdirection' in address_in:
                address_in['mail'] = address_in['mail'] + \
                    ' ' + address_in['stpostdirection']
            if 'unittype' in address_in:
                address_in['mail'] = address_in[
                    'mail'] + ' ' + address_in['unittype']
            if 'unitnum' in address_in:
                address_in['mail'] = address_in[
                    'mail'] + ' ' + address_in['unitnum']
        ...
        if 'mail' in address_in and address_in['mail']:
            ...
            address['mail'] = address_stand.AddressLineStandardization(address_in[
                                                                       'mail'])
        ...
        if csmatch and zipmatch:
            if csmatch['citystatekey'] == zipmatch['citystatekey']:
                ...
                matchrules = [
                    ...
                    {'financenumber': zipmatch[
                        'financenumber'], 'exact': True},
                ]
        ...
        if 'stname' in address and address['stname']:
            for rule in matchrules:
                ...
                if 'zipcode' in rule:
                    ...
                    query.append(
                        "JOIN usps_cs_detail AS cs_detail ON cs_detail.zipcode = zip4_detail.zipcode AND cs_detail.citystatekey = %s")
                    query_params.append(rule['citystatekey'])
                elif 'financenumber' in rule:
                    query.append(
                        "JOIN usps_cs_detail AS cs_detail ON cs_detail.zipcode = zip4_detail.zipcode AND cs_detail.financenumber = %s")
                ...
                if 'stnum' in address and address['stnum']:
                    ...
                    stnum_or = [
                        'zip4_detail.stnumlow <= %s AND zip4_detail.stnumhigh >= %s AND zip4_detail.stparity IN %s']
                    ...
                    if match:
                        ...
                        stnum_or.append(
                            'zip4_detail.stnumlow <= %s AND zip4_detail.stnumhigh > %s AND zip4_detail.stparity IN %s')
                        stnum_or_params = stnum_or_params + \
                            [stnum_sub, stnum_sub, stparity]

                    # Only allow fuzzy matches without a unit number, and with
                    # an exact zipcode
                    if not ('unitnum' in address and address['unitnum']) and 'zipcode' in rule:
                        stnum_or.append(
                            "zip4_detail.stnumlow = '' AND zip4_detail.stnumhigh = '' AND zip4_detail.recordtypecode IN %s")
                    ...
                    query_where.append(
                        "zip4_detail.stnumlow = '' AND zip4_detail.stnumhigh = ''")
                ...
                if 'unitnum' in address and address['unitnum']:
                    ...
                    unitnum_or = [
                        'zip4_detail.unitnumlow <= %s AND zip4_detail.unitnumhigh >= %s AND zip4_detail.unitparity IN %s']
                    unitnum_or_params = [unitnum, unitnum, unitparity]

                    unitnum_or.append(
                        "zip4_detail.unitnumlow = '' AND zip4_detail.unitnumhigh = '' AND zip4_detail.recordtypecode IN %s")
                    ...
                    query_where.append(
                        "zip4_detail.unitnumlow = '' AND zip4_detail.unitnumhigh = ''")
                ...
                unit_or.append(
                    "zip4_detail.unittype = '' AND zip4_detail.recordtypecode IN %s")
                ...
                for key, value in matchweight_params_in.items():
                    matchweight.append(
                        "CAST((zip4_detail." + key + " = %s) AS INTEGER)")
                    matchweight_params.append(value)

                if address['unittype'] == '#':
                    matchweight.append(
                        "CAST((zip4_detail.unittype != '' OR (recordtypecode in ('S', 'H') AND zip4_detail.unittype = '')) AS INTEGER)")

                else:
                    matchweight.append(
                        "CAST((zip4_detail.unittype = %s OR (recordtypecode in ('S', 'H') AND zip4_detail.unittype = '')) AS INTEGER)")
                    matchweight_params.append(address['unittype'])

                query_select.append(
                    "(" + " + ".join(matchweight) + ") AS weight")
                ...
                query_select_params = query_select_params + \
                    [stnum, stnum, unitnum, unitnum, stnum, stnum, unitnum, unitnum,
                        stnum, stnum, stnum, unitnum, unitnum, unitnum, unittype]

                querysql = "SELECT " + ", ".join(query_select) + " " + " ".join(query) + " WHERE " + " AND ".join(
                    query_where) + " ORDER BY weight DESC, subweight DESC, zip4_detail.zipcode4low"
            ...
            if candidate:
                ...
                if address['zipcode4low'] != address['zipcode4high']:
                    address['zipcode4'] = str(
                        int(address['stnum']) - int(address['stnumlow']) + int(address['zipcode4low']))
                else:
            ...
            if 'zipcode4' in address and 'recordtypecode' in address and address['recordtypecode'] == 'H':
                # Secondary
                unitnum_nofrac = re.sub(
                    r' ?[0-9]+\/[0-9]+( ?[A-Z])?$', '', address['unitnum'], re.IGNORECASE)
                unitnum_clean = re.sub(
                    r'[^0-9A-Z]', '', unitnum_nofrac, re.IGNORECASE)
                x_table = [1, 2, 3, 4, 5, 6, 7, 8, 9, 0, 21, 22, 23,
                ...
                if 'unitnumlow' in address and address['unitnumlow'] == '':
                    ...
                    address['deliverypointcode'] = ord(
                        unitnum_clean[-1].upper()) - ord('A') + 73
                elif re.search(r'[0-9]+[A-Z]+$', unitnum_clean, re.IGNORECASE):
                    # Rule 3: Alphanumeric Rule-Trailing Alpha
                    x_alpha = x_table[
                        ord(unitnum_clean[-1].upper()) - ord('A')]
                    matches = re.search(
                        r'([0-9])[A-Z]+$', unitnum_clean, re.IGNORECASE)
                    ...
                    matches = re.match(
                        r'^[^A-Z]*([A-Z])', unitnum_clean, re.IGNORECASE)
